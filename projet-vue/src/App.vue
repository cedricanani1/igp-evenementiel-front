...
export default {
  ...
  methods: {
    addCart(product) {
      if (localStorage.token || !localStorage.token) {
        ...
        if (item) {
          ...
          product.participant !== undefined &&
          product.location !== undefined &&
          product.objects !== undefined &&
          ...
          if (product.to < product.from) {
            ...
            this.cart.push({
              ...
              photo:
                ...
                product.photo[0].path,
              libelle: product.libelle,
              price: product.price,
              days:
                (Date.parse(product.to) - Date.parse(product.from)) /
                1000 /
                60 /
                60 /
                24,
    ...
    getProducts() {
      this.isLoading = true;
      axios
        .get("https://logistique-backend.igp-ci.com/api/products")
        .then((resp) => {
  ...
  computed: {
    ...
    getCategories() {
      axios
        .get("https://logistique-backend.igp-ci.com/api/categories")
        .then((resp) => {
