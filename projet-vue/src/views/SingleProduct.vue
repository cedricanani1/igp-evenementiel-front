...
<div class="Myspinner" v-show="spinner">
  ...
  ></loading> -->
    ...
    <div class="product-details-area pt-100 pb-3" v-if="items.photo">
      <div class="container">
        <div class="top">
                ...
                <div class="row">
                  <div class="col-sm-3 col-lg-3">
                    <div class="owl-thumbs" data-slider-id="1">
                      <div
                        class="item owl-thumb-item"
                        v-for="(item, index) in items.photo"
                        :key="index"
                        v-if="items.photo.length > 0"
                      >
                        <div class="top-img">
                          <img
                            :src="
                  ...
                  <div class="col-sm-9 col-lg-9">
                    <div
                      class="image-slides owl-theme owl-carousel owl-loaded owl-drag"
                    >
                      <div
                        class="item"
                        v-for="(item, index) in items.photo"
              ...
              <div class="top-content">
                ...
                <ul class="reviews badge bg-secondary p-3">
                  ...
                  <div v-if="items.start == 5">
                    <i
                      class="bi bi-star-fill start fs-5"
                      :class="color"
                      title="mauvais"
                    ></i>
                    <i
    ...
    <div class="container-fluid rating mx-auto py-4">
      <form
        @submit.prevent.stop="postRating"
        class="bg-dark rounded form mx-auto px-4 py-3"
      >
        <h1 class="text-light">Noter le produit</h1>
        <ul class="reviews w-100" id="stars">
          <i
            class="bi bi-star-fill start fs-2"
            title="mauvais"
            :id="1"
            @click="rated(1, 1)"
          ></i>
          ...
          <i
            ...
            @click="rated(5, 5)"
          ></i>
          <span class="badge bg-secondary ms-1 p-2"
            >{{ this.rating.rate }} /5</span
          >
        </ul>
        <!-- <input class="my-2" v-model="rating.rate" type="text" name="rate" placeholder="Rating" > <br>  -->
        <input
          class="mb-2 w-100 border-0 rounded"
          v-model="rating.object"
          type="text"
          name="objet"
          placeholder="Objet"
          required
        ...
        <div class="row">
          ...
          >
            <div class="products-item">
              ...
              <i
                class="bi bi-star-fill start float-end me-2 mt-1"
                :class="color"
                v-if="items.start !== 0"
              ></i>
...
export default {
  ...
  methods: {
    get_items() {
      this.spinner = true;
      axios
        .get("https://logistique-backend.igp-ci.com/api/products/" + this.id)
        .then((resp) => {
    ...
    // getBestSeller() {
    //   axios
    //     .post("https://logistique-backend.igp-ci.com/api/bestproduct")
    //     .then((resp) => {
    //       this.bestSeller = resp.data;
    //       // console.log("bestSeller",resp.data);
    //     });
    ...
    postRating() {
      axios
        .post("https://logistique-backend.igp-ci.com/api/rating", this.rating)
        .then((reponse) => {
